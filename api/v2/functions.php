<?php 
function Br_SecureData($config = array(), $data = array()) {
    global $non_allowed;
...
function Br_UserContactsAPP($limit = 20, $json_contacts = '{}', $user_id = 0) {
    ...
    if (!empty($json_decode)) {
        foreach ($json_decode as $key => $contact) {
            $contact_name = Br_Secure($contact['Key']);
            $contact_phone = Br_Secure($contact['Value']);
            ...
            while ($mysqli_fetch = mysqli_fetch_assoc($query)) {
                if (!empty($mysqli_fetch['user_id'])) {
                    $data[] = Br_UserData($mysqli_fetch['user_id']);
                }
            }
        }
    }
...
function Br_GetFilePosition($file) {
    $file_type = 'text';
    if (empty($file)) {
        return $file_type;
    }
