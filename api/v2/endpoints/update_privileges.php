...
if (!empty($_POST['page_id']) && is_numeric($_POST['page_id']) && $_POST['page_id'] > 0 && !empty($_POST['user_id']) && is_numeric($_POST['user_id']) && $_POST['user_id'] > 0) {
    $page_data = Br_PageData($_POST['page_id']);
    if ($page_data['user_id'] == $br['user']['id'] || Br_IsCanPageUpdate($_POST['page_id'],'admins')) {
        ...
        if (Br_UpdatePageAdminData($_POST['page_id'], $update_array,$_POST['user_id'])) {
