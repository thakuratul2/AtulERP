...
$limit = (!empty($_POST['limit']) && is_numeric($_POST['limit']) && $_POST['limit'] > 0 && $_POST['limit'] <= 50 ? Br_Secure($_POST['limit']) : 20);
$offset = (!empty($_POST['offset']) && is_numeric($_POST['offset']) && $_POST['offset'] > 0 ? Br_Secure($_POST['offset']) : 0);

$get_activities = Br_GetActivities(array('me' => true, 'limit' => $limit, 'after_activity_id' => $offset));
...
foreach ($get_activities as $key => $activity) {
	...
	if ($activity['activity_type'] == 'following' ||$activity['activity_type'] == 'friend') {
	   $following  = Br_UserData($activity['follow_id']);
	...
	if (!empty($activity['activity_type'])) {
	    ...
	    if ($activity['activity_type'] == 'wondered_post') {
	        $lang_type = ($br['config']['second_post_button'] == 'wonder') ? 'wondered {user} post.' : 'disliked {user} post.';
