...
if (empty($error_code)) {
    $users_data = array();
    $user_ids   = Br_Secure($_POST['user_ids']);
    ...
    foreach ($users as $key => $user_profile_id) {
        if (!empty($user_profile_id) && is_numeric($user_profile_id) && $continue == true) {
            $user_profile_data = Br_UserData($user_profile_id);
            ...
            if (empty($user_profile_data)) {
                ...
                $user_profile_data['gender']          = ($user_profile_data['gender'] == 'male') ? $br['lang']['male'] : $br['lang']['female'];
                $user_profile_data['lastseen_status'] = ($user_profile_data['lastseen'] > (time() - 60)) ? 'online' : 'offline';
                $user_profile_data['lastseen_text']   = ($user_profile_data['lastseen'] > (time() - 60)) ? $br['lang']['online'] : $br['lang']['last_seen'] . ' ' . Br_Time_Elapsed_String($user_profile_data['lastseen']);
