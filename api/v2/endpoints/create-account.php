...
if (empty($error_code)) {
    ...
    if (in_array(true, Br_IsNameExist($username, 0))) {
        $error_code    = 4;
        $error_message = 'Username is already taken';
    } else if (in_array($username, $br['site_pages']) || !preg_match('/^[\w]+$/', $username)) {
        $error_code    = 5;
    ...
    } else if (Br_EmailExists($email) === true) {
        $error_code    = 7;
    ...
    if (empty($error_code)) {
        ...
        $gender = 'male';
        if (in_array($_POST['gender'], array_keys($br['genders']))) {
            $gender = $_POST['gender'];
        }
        $code = md5(rand(1111, 9999) . time());
        $account_data = array(
            'email' => Br_Secure($email, 0),
            'username' => Br_Secure($username, 0),
            'password' => $password,
            'email_code' => $code,
            'src' => 'Phone',
            'timezone' => 'UTC',
            'gender' => Br_Secure($gender),
            'lastseen' => time(),
            'active' => Br_Secure($activate)
        );
        if (!empty($_POST['android_m_device_id'])) {
            $account_data['android_m_device_id']  = Br_Secure($_POST['android_m_device_id']);
        }
        if (!empty($_POST['ios_m_device_id'])) {
            $account_data['ios_m_device_id']  = Br_Secure($_POST['ios_m_device_id']);
        }
        if (!empty($_POST['android_n_device_id'])) {
            $account_data['android_n_device_id']  = Br_Secure($_POST['android_n_device_id']);
        }
        if (!empty($_POST['ios_n_device_id'])) {
            $account_data['ios_n_device_id']  = Br_Secure($_POST['ios_n_device_id']);
        }
        $register     = Br_RegisterUser($account_data);
        if ($register === true) {
            
            if ($activate == 1) {
                $access_token        = sha1(rand(111111111, 999999999)) . md5(microtime()) . rand(11111111, 99999999) . md5(rand(5555, 9999));
                $time                = time();
                $user_id             = Br_UserIdFromUsername($username);
            ...
            } elseif ($br['config']['sms_or_email'] == 'mail') {
                $user_id             = Br_UserIdFromUsername($username);
                $br['user']        = $_POST;
                $br['code']        = $code;
                ...
                $send_message_data = array(
                    ...
                    'to_email' => $email,
                    'to_name' => $username,
                    'subject' => $br['lang']['account_activation'],
                    'charSet' => 'utf-8',
                    'message_body' => $body,
                    'is_html' => true
                );
            ...
            elseif ($br['config']['sms_or_email'] == 'sms' && !empty($_POST['phone_num'])) {
